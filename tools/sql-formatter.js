document.addEventListener('DOMContentLoaded', () => {
    ...
    function formatCode() {
        ...
        if (!input.trim()) {
            showToast('Please enter some SQL code to format');
            return;
        }
        
        // Get selected language
        let selectedLanguage = 'sql';
        ...
        try {
            const formatted = sqlFormatter.format(input, {
                ...
                keywordCase: uppercase.checked ? 'upper' : 'lower',
                linesBetweenQueries: 2
            });
            
            outputTextarea.value = formatted;
        } catch (error) {
            showToast('Error: Invalid SQL syntax - ' + error.message);
        }
    }
